...
func runReview(cmd *cobra.Command, opts *reviewOptions) error {
	actions := []bool{opts.Start, opts.AddComment, opts.DeleteComment, opts.EditComment, opts.Submit}
	enabled := 0
	...
	if enabled != 1 {
		return errors.New("specify exactly one of --start, --add-comment, --delete-comment, --edit-comment, or --submit")
	}
